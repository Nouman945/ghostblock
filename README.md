...
Chrome MV3 extension that kills the ad layer you cannot see: transparent click catchers over the video player, invisible overlay links, zero-size third-party iframes, link hijacks, and popunders.

Most blockers go after the ads you can see. This one goes after the layer you cannot: the invisible `<div>` sitting over the play button, the 1x1 tracker frame, the link that rewrites itself on click, and the popunder that fires from a click anywhere on the page.

Built and tested against two streaming sites that use a commercial popunder SDK, documented in detail below.
...
- **Toolbar popup**: block count for the tab, four toggles (protection, allow this site, on-page button, debug), a rescan button, and the recent-activity list with the rule that fired on each item. Follows the system light/dark theme.
- **On-page button**: a floating pill, bottom right, top frame only, showing the live block count. Click it to turn Ghostblock off for that site and reload. It sits in a **closed** shadow root so page CSS cannot touch it, and it is excluded from the classifier via `data-gb-ui`. Turn it off in the popup if you don't want it.
- Internal markers are `data-gb` (neutralized element, value is the rule name) and the `__gb_cfg` / `__gb_hit` events between the isolated and main worlds.

## What it does

**1. Popunder blocking** (`src/main-world.js`, runs in the page's own JS world at `document_start`)

- Patches `window.open`. A call is allowed only if the URL is same-site, or matches the href of the link the user actually clicked within the last second. Everything else gets a stub window object so the page's script does not throw.
- Patches `HTMLAnchorElement.prototype.click` for the create-hidden-anchor-and-click trick.
- Neutralizes `onbeforeunload`, used to re-trigger the popunder on navigation.

**2. Hidden element removal** (`src/content.js`)

Runs a classifier on every candidate element, on mutation and on a 1s tick for the first 20s (these sites re-inject).

| Pattern | Detection |
|---|---|
| Zero-size ad frame | iframe/embed/object, third-party src, width or height <= 3px |
| Invisible frame | third-party src + `display:none`, `visibility:hidden`, or `opacity < 0.1` |
| Off-screen frame | third-party src positioned outside the viewport |
| Click catcher | fixed/absolute, covers >70% width and >50% height, transparent, no visible content |
| Full-page overlay | covers the viewport, `z-index > 999`, contains a third-party link |
| Invisible link overlay | `<a>` to a third party with no text, image, or svg inside |

Matched elements get `display:none !important`, `pointer-events:none`, and iframes get `src="about:blank"` so the ad stops loading. Nothing is removed from the DOM, which avoids tripping anti-adblock scripts.

Guards against false positives: skips anything containing a `<video>` or `<canvas>`, skips known-safe third parties (reCAPTCHA, hCaptcha, Turnstile, Stripe, PayPal, YouTube/Vimeo embeds).

A capture-phase click handler also cancels clicks that land on a third-party overlay link that survived the sweep.

**3. Network blocking** (`rules/network.json`)

declarativeNetRequest static rules blocking ~70 ad and popunder networks plus `pop.js` / `popunder` / `aclib` URL patterns. `main_frame` is excluded so a blocked domain never breaks direct navigation.
...
Toggle it in the popup. Suspicious elements get a red outline and a tooltip with the reason instead of being hidden. Use it on a site to check for false positives before trusting the blocking.

The popup lists the last 40 things blocked on the tab (reason + tag + id/class), which is what you use to tune the selectors in `src/hide.css` and the thresholds in `classify()`.

## Per-site allowlist

The "Allow this site" checkbox adds the host to `allowlist` in storage and installs a `allowAllRequests` session rule so the network rules stand down too.

One caveat: the content script reads storage asynchronously, so on an allowlisted site `window.open` is patched for a few ms before the stand-down message arrives. Harmless in practice, no clean fix in MV3.

## Tuning

- `SAFE_HOSTS` in `src/content.js` — third parties that legitimately load hidden frames.
- `classify()` thresholds — the 0.7/0.5 viewport coverage and `z-index > 999` numbers.
- `rules/network.json` — add domains you see in DevTools Network.
- `src/hide.css` — static cosmetic rules, applied before any JS runs.

## Limits

- Rule IDs 1-6 are static, 10000+ are session allow rules. Keep them apart if you add more.
- `baseDomain()` takes the last two labels, so `foo.co.uk` and `bar.co.uk` read as same-site.
- No icons, Chrome shows the default puzzle piece. Drop 16/48/128 PNGs in and add an `icons` block to the manifest if you want one.
- This handles the hidden layer. For the visible banner ads, run it alongside uBlock Origin.
